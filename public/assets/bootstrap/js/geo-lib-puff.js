...
function getQueriesForDocumentsAround(ref, center, radiusInKm, geoField, common_query_str) {
    ...
    return geohashesToQuery.map(function (location) {
        return ref.where(geoField, ">=", location[0]).where(geoField, "<=", location[1]).where('custom_query_array', 'array-contains', common_query_str);
    });
}
